...
import {
	...
	supportBuildDirection,
	supportMountPlane
} from './levels.js';
import { cutoutPoints, openingCutPoints } from './geometry.js';
import { joistSides } from './perimeter.js';
import { supportAssemblyOrigin } from './mounting.js';
import { packagingView } from './view.js';
import { trayMetrics, trayHasPull, trayPullWidthAtMouth } from './supports.js';
 ...
 * the wall taper.
 */
export function trayAssemblyProfile(
	tray: Support,
...
export function supportFlangeDescriptors(
	...
	const foldOutward =
		assemblyFoldDirection(`riser:${support.id}`, 'riser-bottom-flange-fold', settings) === 'down';
	const outward = support.kind === 'platform' ? false : foldOutward;
	return [
		{
			side: 'bottom',
			a: point(0, 0),
...
export function buildAssembly(
	...
	// --- Pocket walls: the flaps folded out of each opening -----------------
	for (const pocket of design.pockets) {
		if (design.fabricationMode === 'router') continue;
		if (!SIDES.some((side) => pocket.sides[side])) continue;
		const x = pocket.x - design.deckX;
		const y = pocket.y - design.deckY;
		const owner = `pocket:${pocket.id}`;
		const wallSign = assemblyFoldSign(owner, 'top-fold', design);
		// A wall folded down cannot reach past the surface the deck stands on.
		...
		const walls: Record<Side, { edges: Parameters<typeof wall>[2]; hinge: [Point, Point, Point] }> =
			{
				left: {
					edges: {
...
function buildSupportGroup(
	...
	parts.push(
		box(0, -t / 2, wallBaseZ, support.w, t, wallHeight),
		box(0, support.d - t / 2, wallBaseZ, support.w, t, wallHeight),
		box(-t / 2, 0, wallBaseZ, t, support.d, wallHeight),
		box(support.w - t / 2, 0, wallBaseZ, t, support.d, wallHeight),
		box(0, 0, panelZ - t / 2, support.w, support.d, t)
	);
	...
	const tabInward = assemblyFoldDirection(owner, closureRole, design) === 'down';
