...
describe('spanning heights', () => {
	it('fills the cavity from the box floor up to the deck underside', () => {
		...
		const base = design({ supports: [spanning] });
	...
	it('follows the wall height, which is the point of spanning', () => {
		...
		const tall = design({ supports: [spanning], perimeterWall: 55 });
		expect(resolveSupportHeight(spanning, view(tall))).toBe(55);
	});

	it('follows the joist height when the perimeter is a joist', () => {
		const spanning = riser({ heightMode: 'span' });
		const joist = design({ supports: [spanning], perimeterType: 'joist', joistHeight: 22 });
	...
	it('spans only the gap left above its mount offset', () => {
		...
		expect(resolveSupportHeight(spanning, view(design({ supports: [spanning] })))).toBe(25);
	});
	...
	it('spans the remaining gap when stacked on another support', () => {
		...
		const stacked = design({ supports: [lower, upper] });
	...
	it('leaves a fixed height alone', () => {
		...
		expect(resolveSupportHeight(fixed, view(design({ supports: [fixed] })))).toBe(17);
	});
	...
	it('clamps to zero rather than going negative above the deck', () => {
		...
		expect(resolveSupportHeight(impossible, view(design({ supports: [impossible] })))).toBe(0);
	});
	...
	it('offers a span only from a surface that has the deck above it', () => {
		const from = (mount: Support['mount'], extra: Support[] = []) => {
			...
			return canSpanToDeck(support, view(design({ supports: [support, ...extra] })));
		};
	...
	it('judges a stack by where its host actually sits, not by the kind of anchor', () => {
		...
		expect(
			canSpanToDeck(
				stackedOn('inside'),
				view(design({ supports: [inside, onDeck, stackedOn('inside')] }))
			)
		).toBe(true);
		...
		expect(
			canSpanToDeck(
				stackedOn('on-deck'),
				view(design({ supports: [inside, onDeck, stackedOn('on-deck')] }))
			)
		).toBe(false);
	});
});
...
describe('resolveSupportHeights', () => {
	it('writes the derived height back into the document', () => {
		...
		const resolved = resolveSupportHeights(design({ supports: [spanning] }));
		expect(packagingData(resolved).supports[0]?.h).toBe(40);
	});

	it('returns the same design when nothing spans, so history stays cheap', () => {
		const base = design({ supports: [riser()] });
	...
	it('resolves a stack in one pass, whatever order it is stored in', () => {
		...
		const resolved = resolveSupportHeights(design({ supports: [upper, lower] }));
		expect(packagingData(resolved).supports.find((r) => r.id === 'lower')?.h).toBe(40);
		// The lower riser already fills the cavity, so nothing is left to span.
		expect(packagingData(resolved).supports.find((r) => r.id === 'upper')?.h).toBe(0);
	});

	it('does not mutate the design it is given', () => {
		const base = design({ supports: [riser({ heightMode: 'span', h: 1 })] });
...
describe('dropping a support', () => {
	...
	it('stacks on the support it lands on top of', () => {
		...
		const base = design({ supports: [lower, dragged] });
		// Centre of the dragged part lands inside the lower riser's footprint.
		const drop = resolveDrop(dragged, 80, 60, dragView(base));
	...
	it('keeps its base anchor when it lands on nothing', () => {
		const dragged = riser({ id: 'top', w: 40, d: 30 });
		const drop = resolveDrop(dragged, 5, 5, dragView(design({ supports: [lower, dragged] })));
	...
	it('falls back to the base its stack stood on when dragged clear', () => {
		...
		const base = design({ supports: [onDeck, stacked] });
		// Dragged well clear of the host it was standing on.
		const drop = resolveDrop(stacked, 300, 300, dragView(base));
	...
	it('clamps the placement to the surface it landed on', () => {
		...
		const drop = resolveDrop(dragged, 135, 115, dragView(base));
		expect(drop.mount).toEqual({ anchor: 'support-top', supportId: 'lower', offset: 0 });
	...
	it('never stacks a tray, which is a hole rather than a surface', () => {
		...
		const base = design({ supports: [lower, tray] });
		const drop = resolveDrop(tray, 80, 60, dragView(base));
	...
	it('drops onto the highest surface when stacks overlap', () => {
		...
		const host = supportUnderPoint(
			...
			80,
			60,
			dragView(design({ supports: [lower, tall, dragged] }))
		);
	...
	it('turns a span into a fixed height when it lands where it cannot span', () => {
		...
		const base = design({ supports: [onDeck, spanning] });
		...
		const drop = resolveDrop(spanning, 90, 70, dragView(base));
	...
	it('keeps a span when it lands on a support inside the box', () => {
		...
		const base = design({ supports: [inside, spanning] });
		expect(resolveDrop(spanning, 90, 70, dragView(base)).heightMode).toBe('span');
	});
...
describe('vertical validation', () => {
	const withRisers = (...supports: Support[]) =>
		validate(
			design({
				supports,
				sheets: [
...
describe('machine profiles across a packaging job', () => {
	const twoSheets = (): DesignState =>
		patchDesign(design(), {
			...
			sheets: [
				{ id: 'deck', name: 'Deck', workspace: 'packaging', machineProfileId: 'knife' },
				{ id: 'parts', name: 'Parts 1', workspace: 'packaging', machineProfileId: 'knife' }
			],
			supports: [riser({ id: 'r1', sheetId: 'parts', flatX: 300, flatY: 300 })]
		});
	...
	it('ignores an empty sheet, which carries no parts to disagree about', () => {
		...
		expect(
			validate({
				...
				sheets: [
					...mixed.sheets,
					{ id: 'spare', name: 'Spare', workspace: 'packaging', machineProfileId: 'router' }
				]
			})
	...
	it('cuts each sheet on its own machine, so geometry follows the profile', () => {
		const routed = withMachine(design({ supports: [] }), { fabricationMode: 'router' });
		// A router never folds, so it emits no score paths.
		expect(allGeometry(routed).paths.some((path) => path.type === 'score')).toBe(false);
		expect(allGeometry(design({ supports: [] })).paths.some((path) => path.type === 'score')).toBe(
			true
		);
	});
});
